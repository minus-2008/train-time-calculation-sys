  ...
  while True: #停車通過パターン読み込み
    ...
    if pattern_data=='S': #Sのときすなわち停車時に計算かつクリア
      ...
      for sta in reversed(sta_passing):#駅構内制限処理
        ...
        print(lim_adi)
        flag_backrange=True
        index_start=bisect.bisect_right(limit_range[0],sta_loc+lim_adi[0][0])
        ...
        for point_talp in reversed(lim_adi): #番線別制限速度追加(後ろから)
          ...
          if point_loc>=limit_range[0][-1]:#制限地点が計算範囲外(後ろ)の場合
            ...
            for i in range(index_start,index_end):#すでに変更区間内にある制限をstrに書き換え(のちに削除を行う)
      ...
      for i in range(1,len(limit_range[0])):
