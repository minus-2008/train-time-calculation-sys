...
def gra_get(location,gra_list):#勾配検索
  index_=bisect.bisect_right(gra_list[1],location)-1
  gra=gra_list[0][index_]
  return gra/1000#10‰▷0.01
...
gravity=float(read_API_acell(cars_meta,9,2)) #gravity▷重力加速度
    ...
    if pattern_data=='S': #Sのときすなわち停車時に計算かつクリア
      ...
      for sta in reversed(sta_passing):#駅構内制限処理
        ...
        for i in range(index_start,index_end):#すでに変更区間内にある制限をstrに書き換え(のちに削除を行う分)
          if limit_range[1][i+1]!=0:#最終項、初項を書き換えないように
      ...
      for section in section_list:
        ...
        while Flag_acceleration:#加速本処理
          ...
          gra=weight*gravity*gra_get(location,gra_list)#重力
          force=float(car_acclist[int(roundspeed/car_dv_acc)][car_type])-gra#force▷加速力N　参照
      ...
      pyp.plot(acc_curve[1],acc_curve[2],'#0000ff')
      

      #減速計算ループ
      
      cum_dec_distance=0#累積減速距離(惰行減速を除く)
      dec_curve=[[],[],[]]#dec_curve▷減速曲線[[速度][位置][加速時間]]
      for section in reversed(section_list):
        ...
        while Flag_deceleration:
          ...
          gra=weight*gravity*gra_get(location,gra_list)#重力
          force=float(car_declist[int(roundspeed/car_dv_dec)][car_type])+gra#+gravity_force#force▷加速力N　参照
      ...
      pyp.plot(dec_curve[1],dec_curve[2],'#0000ff')
      #pyp.show()
      ...
      for section in section_list:
        ...
        index_start=bisect.bisect_left(acc_curve[1],start_point)
        index_end=bisect.bisect_right(acc_curve[1],end_point)
        if index_start==index_start:#加速区間が無い場合、キャンセル
          Flag_accdec=False
        area_acc_curve=[acc_curve[0][index_start:index_end],acc_curve[1][index_start:index_end],acc_curve[2][index_start:index_end]]
        index_start=bisect.bisect_left(dec_curve[1],start_point)
        index_end=bisect.bisect_right(dec_curve[1],end_point)
        if index_start==index_end:#減速区間が無い場合、キャンセル
          Flag_accdec=False
        area_dec_curve=[dec_curve[0][index_start:index_end],dec_curve[1][index_start:index_end],dec_curve[2][index_start:index_end]]

        if not Flag_accdec:#area_acc_curve[0]が空だった際、エラー起きるので
          pass
        elif max_speed>max(area_acc_curve[0]) and max_speed>max(area_dec_curve[0]):
          Flag_accdec=False
        
        acc_area=[]
        for index_,time_ in enumerate(acc_curve[2]):
          if time_==0 and type(time_)==int:
            acc_area.append(index_)
        print('acc_area',acc_area)
        dec_area=[]
        for index_,time_ in enumerate(dec_curve[2]):
          if time_==0 and type(time_)==int:
            dec_area.append(index_)
        print('dec_area',dec_area)
        print(acc_curve[2][0:100])
        ...
        while Flag_accdec:
          ...
          if speed in area_acc_curve[0] and speed in area_dec_curve[0]:
            index_acc=area_acc_curve[0].index()
          Flag_accdec=False
