    ...
    if pattern_data=='S': #Sのときすなわち停車時に計算かつクリア
      ...
      for index_,time_ in enumerate(acc_curve[2]):
        if time_==0 and type(time_)==int:
          acc_area.append(acc_curve[1][index_])
      acc_area.append(float('inf'))
      print('acc_area',acc_area)
      dec_area=[]
      for index_,time_ in enumerate(dec_curve[2]):
        if time_==0 and type(time_)==int:
          dec_area.append(dec_curve[1][index_])
      dec_area.append(-float('inf'))
      dec_area=list(reversed(dec_area))
      print('dec_area',dec_area)
      accdec_section=[]
      for i in range(len(acc_area)-1):
        i_this=bisect.bisect_left(dec_area,acc_area[i])
        i_next=bisect.bisect_left(dec_area,acc_area[i+1])
        if i_this==i_next:#この項と次の項の間に数値があるかを確認し、ない場合は加速間の惰行区間を登録する
          sat_p=acc_area[i]
          end_p=acc_area[i+1]
          coast_type='acc_acc'
          accdec_section.append([sat_p,end_p,coast_type])
        else:#この項と次の項の間に数値がある場合は、加減速区間処理登録、などをする
          sat_p=acc_area[i]
          end_p=dec_area[i_this]
          coast_type='acc_dec'
          accdec_section.append([sat_p,end_p,coast_type])
          for j in range(i_next-i_this-1):
            sat_p=dec_area[i_this+j]
            end_p=dec_area[i_this+j+1]
            coast_type='dec_dec'
            accdec_section.append([sat_p,end_p,coast_type])
          sat_p=dec_area[i_next-1]
          end_p=acc_area[i+1]
          coast_type='dec_acc'
          accdec_section.append([sat_p,end_p,coast_type])
      accdec_section.pop() #末項は停車後の処理となるため削除


      for section in accdec_section:
        sat_p=section[0]
        end_p=section[1]
        coast_type=section[2]
        if coast_type=='acc_acc':
          continue
        elif coast_type=='acc_dec':
          continue
        elif coast_type=='dec_dec':
          continue
        elif coast_type=='dec_acc':
          continue
        else:
          raise ValueError('内部処理に不適切な処理が発生しました。発生個所：coast_type',coast_type)
        #もう少しエレガントな解決策を考えます
