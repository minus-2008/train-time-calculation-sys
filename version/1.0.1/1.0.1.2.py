...
while True:#メインループ
  ...
  while True: #停車通過パターン読み込み
    ...
    if pattern_data=='S': #Sのときすなわち停車時に計算かつクリア
      ...
      for index,limspead in enumerate(lim_list[0][limit_start:limit_end+1]):
      ...
      for sta in sta_passing:#駅構内制限処理
        _sta=stanum_dec[sta[0]][2]
        loc=stanum_dec[sta[0]][1]-running_range[0]
      ...
      for i in range(len(limit_range[0])-1):
        length=limit_range[0][i+1]-limit_range[0][i]
        start_speed=limit_range[1][i]
        max_speed=limit_range[1][i+1]
        end_speed=limit_range[1][i+2]
        section.append([length,start_speed,max_speed,end_speed])
      print(section)
      print('処理の書き換えが完了しました')
