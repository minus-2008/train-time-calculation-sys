...
i=1#i▷繰り返し変数
car_dv_acc=float(cars1.cell(3,1).value)-float(cars1.cell(2,1).value)#car_dv_acc▷加速記述間隔 初項と次項の差から計算
car_dv_dec=float(cars2.cell(3,1).value)-float(cars2.cell(2,1).value)#car_dv_dec▷減速記述間隔 初項と次項の差から計算
car_fainalspeed=int(cars_meta.cell(5,2).value)#car_fainalspeed▷演算最高速度
while True:
  i=i+1
  temp=cars_meta.cell(1,i).value
  if temp=='E':#Eの場合、終了指示
    print(car_typelist)
    break
  else:
...
#車両加減速設定
car_weight=cars_meta.get('B2:'+str(cell_exchange(2+len(car_typelist),3)))#車両重量リスト
car_length=cars_meta.get('B3:'+str(cell_exchange(2+len(car_typelist),4)))#車両長リスト
car_acclist=cars1.get('B5:'+str(cell_exchange(len(car_typelist)+1,2+int(car_fainalspeed/car_dv_acc))))#car_acclist▷車両加速リスト
car_declist=cars2.get('B5:'+str(cell_exchange(len(car_typelist)+1,2+int(car_fainalspeed/car_dv_dec))))#car_declist▷車両減速度リスト
...
for column in range(runningrange_number):#メインループ
  ...
  while True: #停車通過パターン読み込み
    ...
    if pattern[-1]=='F':#Fで改行
      print('F')
      break
    target.update_cell(line,running_column,'↓')#読んだ行に書き込み
