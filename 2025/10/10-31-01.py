...
for loc,typ,data in event_data:#event_all_listの1つ目loc,2つ目typ,3つ目dataが格納される
  if typ in event_dec:#typがevent_decの'aa'の中に当てはまる場合
    ...
    _list[1].append(loc)#場所を記録
    _list[0].append(data)#情報を記録

sta_data=[]
for stanum in sta_list[0]:#駅情報をリスト、それを引き出す辞書型で構成する
  sta_data.append([stanum,[]])
...
for i,track in enumerate(sta_datasorce):#読み込む行
  ...
  _list[1].append([track[1]])#番線名を格納
  _list[1][-1].append([int(x) for x in sta_datasorce[i][2:6]])#単純加算情報を格納
  _list[1][-1].append([[],[]])#速度制限用の空配列を格納
  for j,(limspead,limplace) in enumerate(zip(sta_datasorce[i][6::2],sta_datasorce[i][7::2])):#2つ抜き出し
    if limspead=='':#情報が記載されていないなら
      pass#パス
    else:
      _list[1][-1][-1][0].append(int(limplace))#制限地点を格納
      if limspead=='ED':
        _list[1][-1][-1][1].append(float('inf'))#制限速度を格納(速度解除標識を無限と書き換え)
      else:
        _list[1][-1][-1][1].append(int(limspead))#制限速度を格納
print(sta_data)
exit()
#↓は駅情報の格納形式(一例)
#[['MT09', [['1', [20, 0, 20, 0], [[-400, -300, 50], [50, 45, inf]]],
#           ['2', [0, 0, 0, 0], [[], []]],      　　　　　　　　　　　これは本線
...
# ['MT07', []],　　　　　　　　　　　　　　　　　　　　　　 　　　　　　　　　　　これは使用番線が1つのみの駅
# ['MT06', []],　　　　　　　　　　　　　　　　　　　　　　 　　　　　　　　　　　これは使用番線が1つのみの駅
# ['MT05', [['1', [20, 0, 20, 0], [[-400, 50], [50, inf]]],
#           ['2', [0, 0, 0, 0], [[], []]],      これは本線
#           ['3', [20, 0, 20, 0], [[-400, 50], [50, inf]]]]],
# ['MT04', []],　　　　　　　　　　　　　　　　　　　　　　 　　　　　　　　　　　これは使用番線が1つのみの駅
# ['MT03', []],　　　　　　　　　　　　　　　　　　　　　　 　　　　　　　　　　　これは使用番線が1つのみの駅
# ['MT02', []],　　　　　　　　　　　　　　　　　　　　　　 　　　　　　　　　　　これは使用番線が1つのみの駅
# ['MT01', [['1', [20, 0, 20, 0], [[-400, 50], [50, inf]]],
#           ['2', [0, 0, 0, 0], [[], []]],      　　　　　　　　　　　これは本線
#           ['3', [20, 0, 20, 0], [[-400, 50], [50, inf]]]]]]
